...
администратора). Технический test-baseline репозитория — `39d355c` (см.
"Качество и верификация" выше). Production-деплой 2026-09-12 на текущем
VPS зафиксирован на коммите `9b06ad6` (production deployment/acceptance
baseline; документационные коммиты поверх `39d355c`, код и тесты не
меняющие) и подтверждён ручной приёмкой деплоя/production-smoke-проверкой
(готовность сервисов, аутентификация, доступность SPA, Nginx-периметр,
TLS/автопродление, Registry auth — см. "Статус проекта" → "Текущая
production-приемка" ниже); эта приёмка **не повторяла** полную
функциональную E2E-проверку (CRUD услуг, заявки, поведенческая аналитика,
приоритизация) — та зафиксирована отдельно, как историческое
свидетельство на прежней, уже замененной VPS-инфраструктуре, на коммите
`7761901` (см. "Историческая VPS-приемка" ниже).
...
- Ubuntu 24.04 LTS, Docker 29.8.0, Docker Compose plugin v5.5.1 (текущий
  production VPS — проверено на приемке 2026-09-12, см. "Статус проекта")
- Публичный домен: `vibe.elivcloud.org`
- Registry: `registry-vibe.elivcloud.org`
- UFW пропускает снаружи только 22, 80, 443; дополнительно активен
  Fail2ban; SSH — только по ключу, вход под `root` по SSH отключён
...
Этот раздел разделяет несколько фактов, которые легко перепутать: что
реально проверено вручную на живом production VPS **сейчас**, что
относится к прежней (уже замененной) production-инфраструктуре, и что
просто существует в текущем репозитории независимо от VPS.

**Актуальная production-приемка** — 2026-09-12, деплой зафиксирован на
коммите `9b06ad6` (production deployment/acceptance baseline), выполнена
на текущем VPS после переноса production-инфраструктуры на новый хост
(см. "Текущая production-приемка" и "Проверка legacy-миграции" ниже).
Документационные коммиты поверх технического test-baseline `39d355c`
(включая `9b06ad6`) код и тесты не меняют, поэтому тестовые результаты,
полученные прогоном на `39d355c` (2026-09-05, см. "Локальная разработка и
тесты"), остаются актуальными и для этого baseline.

**Более ранняя ручная production-приемка**, зафиксированная на commit
`7761901`, выполнялась на прежнем VPS, с которого позже производилась
миграция — сохранена ниже как историческое свидетельство ("Историческая
VPS-приемка") и не переписана под текущее состояние.

### Текущая production-приемка (2026-09-12, commit `9b06ad6`)

Production-инфраструктура перенесена на новый VPS 2026-09-12 (тот же
публичный домен `vibe.elivcloud.org`/`registry-vibe.elivcloud.org`);
`main` и `origin/main` были синхронизированы, рабочее дерево чисто на
момент деплоя.

**Инфраструктура (текущий VPS):**

- Ubuntu 24.04 LTS, Docker 29.8.0, Docker Compose plugin v5.5.1,
  PostgreSQL 16.15.
- UFW + Fail2ban активны; SSH только по ключу, вход `root` по SSH
  отключён.
- `postgres`/`backend`/`nginx` — `healthy`, `registry` — running, у всех
  сервисов итоговый `RestartCount=0`.
- `db-roles-bootstrap`/`db-migrate`/`db-roles-finalize` — `Exited (0)`.
- Watchtower в стеке отсутствует (удалён ещё в Stage 3, см. "Почему так" —
  подтверждено и на этом VPS).

**Релиз:**

- Backend-образ собран вне VPS. Поскольку это перенос на новый VPS, где
  Nginx/TLS/Registry на момент доставки ещё не были подняты (Registry
  снаружи физически недостижим без Nginx — см. "Первый деплой
  (bootstrap)"), первоначальная доставка выполнена тем же путём, что и
  самый первый исторический деплой: `docker save` → `scp` → проверка
  SHA-256 контрольной суммы архива → `docker load` → сверка Image Id,
  затем локальный тег `vibe-order-infra-backend:latest` и
  `docker compose up -d --no-build`. Только после того как Nginx/TLS/
  Registry были подняты, тот же immutable-тег отдельно запушен в
  приватный Registry и выкачан обратно как smoke-проверка канала Registry
  для будущих обновлений (не как способ первоначальной доставки на этот
  VPS) — идентичность образа совпала:
  `sha256:2ddd6e8d18a809b402785a6976708327648e9488f8f8c10281b9749952f3e360`.
- Frontend production build также собран вне VPS и опубликован атомарно
  (см. "Production build frontend").

**TLS:** Let's Encrypt SAN-сертификат на оба домена, выпущен вне
Docker-стека и смонтирован в Nginx read-only — как и раньше (репозиторий
сам сертификаты по-прежнему не выпускает). Дополнительно на этом VPS
настроено и проверено автопродление: `certbot.timer`, webroot-
аутентификатор через `nginx/acme-challenge`, deploy hook, проверяющий
конфигурацию Nginx перед reload; `certbot renew --dry-run
--run-deploy-hooks` пройден успешно. Подробнее —
[docs/DEPLOYMENT.md](docs/DEPLOYMENT.md), раздел "TLS / reverse-proxy".

**Проверено вручную (production acceptance):**

- HTTPS отвечает; HTTP редиректит на HTTPS.
- `GET /api/health` — healthy.
- Вход администратора работает; `GET /api/auth/check` подтверждает
  наличие администратора; JWT login успешен; `GET /api/auth/me` — `200` с
  токеном, `401` без токена.
- `GET /api/admin-settings/active` — `200`.
- `/` и `/admin` — `200`.
- `/docs`, `/openapi.json`, `/redoc` наружу не публикуются (`404`).
- Registry: `GET /v2/` без credentials — `401`; полный push/pull smoke с
  аутентификацией пройден, идентичность выкачанного образа сверена с
  запушенной.
- Перезагрузка хоста (проверена до поднятия Nginx/Registry): хост
  загрузился в ожидаемое ядро, Docker активен, PostgreSQL и backend —
  `healthy`, lifecycle one-shots (`db-roles-bootstrap`/`db-migrate`/
  `db-roles-finalize`) остались в статусе успешно завершённых
  (`Exited (0)`). Nginx/Registry в этой конкретной проверке не
  поднимались — их состояние после перезагрузки отдельно этим тестом не
  проверялось.

### Проверка legacy-миграции (2026-09-12)

В рамках той же инфраструктурной миграции отдельно, вживую проверен путь
"усыновления" реальной прежней production-базы (не синтетических данных)
— процедура из [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md), раздел
"Существующая (legacy) production-база":

- Источник — реальная production-база с прежнего VPS: PostgreSQL 16.14,
  ровно 4 таблицы (`admins`=1, `admin_settings`=3, `applications`=12,
  `behavior_metrics`=10), без таблицы `alembic_version` — по-настоящему
  домиграционная legacy-база.
- `pg_dump` (custom format) снят и перенесён с проверкой SHA-256
  контрольной суммы после передачи.
- Восстановление и усыновление выполнены **не на production**: отдельный
  одноразовый PostgreSQL 16.15 контейнер, отдельная сеть, отдельный
  volume, без публикации порта — production-база и production volume не
  использовались и не изменялись.
- `adopt_legacy` подтвердил точный фингерпринт, `alembic upgrade head`
  довёл схему с `0001_legacy_baseline` до текущей
  `0004_stage4_priority_score` без изменения количества строк;
  мигрированная схема `applications` включает `service_id` и `NOT NULL
  priority_score`.
- Least-privilege runtime-роль после финализации подтверждена отдельно:
  `SELECT`/`INSERT`/`UPDATE`/`DELETE` на `applications` — есть; `CREATE`
  на схему и `SELECT` на `alembic_version` — нет.
- Изолированные контейнер/сеть/volume удалены после проверки; дамп и его
  контрольная сумма сохранены на новом VPS в отдельном каталоге с правами
  только для root, как свидетельство проверки.

Детальная операционная evidence-запись — [docs/RUNBOOK.md](docs/RUNBOOK.md).

### Историческая VPS-приемка на прежнем VPS (commit `7761901`)

Эта приемка выполнялась на прежнем production VPS проекта, до
инфраструктурной миграции 2026-09-12 (см. "Текущая production-приемка"
выше).
...
- **Инфраструктура**: `postgres:16.14-alpine` и собственный образ
  ...
  удалён позже, в Stage 3 (см. "Состояние репозитория" ниже).
- **HTTPS**: Let's Encrypt сертификат на один SAN на оба домена
  ...
  сознательно его не задавала (добавлен позже, в Stage 4, см. "Состояние
  репозитория" ниже).
...
- **Backend + frontend, включая административную панель, JWT-аутентификацию,
  ...
  **задеплоены на production VPS** по состоянию на commit `7761901`.
...
### Состояние репозитория (технический baseline `39d355c`; production-baseline деплоя `9b06ad6`)

Этот подраздел описывает само состояние репозитория (какой код существует
и чем он покрыт) независимо от VPS. Какие конкретно пункты из этого были
проверены вживую на текущем VPS 2026-09-12 — см. "Текущая
production-приемка" выше (deployment/smoke-приемка: готовность сервисов,
аутентификация, доступность SPA, Nginx-периметр, TLS, Registry auth) — не
весь список ниже переверялся на VPS как часть этой приемки; например,
числа тестового покрытия ниже — результат отдельного локального/CI
прогона (см. "Локальная разработка и тесты"), а не VPS-специфичной
проверки.
...
- **HSTS** включен (Stage 4) в текущей конфигурации Nginx — см. "Security
  notes / ограничения" ниже за точной конфигурацией заголовка. На
  исторической VPS-приемке `7761901` (см. выше) HSTS ещё не был включен.
- Backend + frontend покрыты тестами локально: backend suite на реальной
  ...
  (технический baseline `39d355c`, 2026-09-05 — см. "Локальная разработка и
  тесты"/"CI"). Документационные коммиты поверх `39d355c` (включая
  production-baseline деплоя `9b06ad6`) код/тесты не меняют, поэтому эти
  числа остаются актуальными.

Автопродление сертификата Let's Encrypt настроено и проверено на текущем
production VPS (`certbot.timer`, см. "Статус проекта" → "Текущая
production-приемка" выше и [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)) — это
операционная настройка хоста, сам репозиторий её по-прежнему не
устанавливает.
...
изначально были пройдены на прежнем (уже замененном) VPS при самом первом
деплое проекта, предшествовавшем исторической VPS-приемке на commit
`7761901` (см. "Статус проекта" выше) — на тот момент цепочка Stage 2
database lifecycle (`db-roles-bootstrap` → `db-migrate` →
`db-roles-finalize`) в репозитории ещё не появилась, поэтому та приемка её
не касалась.

На текущем VPS (см. "Статус проекта" → "Текущая production-приемка") эта
же процедура была пройдена заново 2026-09-12, при переносе на новую
инфраструктуру, уже в её нынешнем виде, включая шаг 9 ниже: цепочка
`db-roles-bootstrap` → `db-migrate` → `db-roles-finalize` → `backend`
подтверждённо отработала — все три one-shot сервиса завершились
`Exited (0)`. Процедура ниже в целом остается здесь как инструкция для
повторного/дополнительного bootstrap (например, при развертывании на
будущем новом VPS). Registry **не считается готовым к запуску** без файла
`registry/auth/htpasswd` — до его появления контейнер стартует, но любой
запрос к Registry будет отклонен на этапе аутентификации (это ожидаемо и
правильно, а не баг).
...
14. Smoke tests — актуальная процедура приёмки живёт в
    ...
    checklist") зафиксирован на commit `7761901` и для сегодняшнего кода
...
### Финальная production-приемка (по состоянию на commit `7761901`)

Это более ранняя ручная приемка на прежнем (уже замененном) production
VPS — актуальная приемка на текущем VPS выполнена 2026-09-12 (см. "Статус
проекта" → "Текущая production-приемка" выше). Эта запись не
переписывается под более новый HEAD или под новую инфраструктуру, она
фиксирует то, что было реально проверено на том VPS на момент commit
`7761901`.
...
выше за полной route-матрицей; исторический smoke-test этой
функциональности — "Финальная production-приемка" выше (commit `7761901`,
прежний VPS), актуальный — "Статус проекта" → "Текущая
production-приемка" (2026-09-12, текущий VPS).

## Historical production smoke checklist (commit `7761901`)
...
commit `7761901` — более ранняя ручная VPS-приемка, выполненная на
прежнем (уже замененном) VPS (см. "Статус проекта" выше). Все пункты ниже
подтверждены на том VPS по состоянию на тот момент (см. также "Финальная
production-приемка" выше) — это не чеклист ни для текущего состояния
репозитория, ни для текущего VPS; актуальный чеклист приёмки — в
[docs/DEPLOYMENT.md](docs/DEPLOYMENT.md), актуальная приемка (2026-09-12,
commit `9b06ad6`, текущий VPS) — в "Статус проекта" → "Текущая
production-приемка" выше:
...
(зафиксирован как commit `7761901` — более ранняя ручная VPS-приемка на
прежнем VPS, см. "Статус проекта" выше) — см. "Финальная
...
- **Production delivery**: ни один production release, включая самый
  первый на каждом VPS, не собирается на VPS. Для **обновлений на уже
  работающей инфраструктуре** (Nginx/TLS/Registry подняты и Registry
  снаружи достижим) production release доставляется через private
  Registry — release image собирается вне VPS для `linux/amd64` и
  ...
  --no-build` (см. "Порядок деплоя" ниже) — так доставлялся релиз,
  зафиксированный как commit `7761901` (более ранняя ручная VPS-приемка
  на прежнем VPS, см. "Статус проекта" выше), и так доставляется каждый
  последующий routine-релиз backend.

  **Первоначальный bootstrap на каждом новом VPS — другой канал.** Когда
  на VPS еще нет ни Nginx, ни TLS, ни работающего Registry — это верно и
  для самого первого исторического деплоя на прежнем VPS, и для переноса
  на текущий VPS 2026-09-12 (оба случая, см. "Первый деплой (bootstrap)"
  выше) — Registry снаружи физически недостижим (Registry становится
  снаружи доступен только через Nginx, а Nginx на этом этапе еще не
  поднят), поэтому `docker pull` в принципе не вариант. В обоих случаях
  использовался тот же принцип "образ собран вне VPS + детерминированный
  локальный тег + `--no-build`", но доставка шла через `docker
  save`/`scp`/`docker load` с проверкой SHA-256 контрольной суммы архива и
  сверкой Image Id после `load`. Уже после того как на текущем VPS были
  подняты Nginx/TLS/Registry, тот же immutable-тег backend дополнительно
  был запушен в Registry и выкачан обратно (см. "Статус проекта" →
  "Текущая production-приемка") — это отдельная smoke-проверка канала
  Registry для будущих обновлений, а не способ, которым образ изначально
  попал на этот VPS.
...
production-baseline деплоя `9b06ad6` — коммиты между ними
документационные, код не меняют). Production-приемка 2026-09-12 (см.
"Статус проекта" → "Текущая production-приемка") подтвердила деплой и
smoke-acceptance на этом baseline в объёме своего чеклиста — не каждый
пункт ниже переверялся отдельно как часть этой приемки:
...
- Admin-роут (frontend `/admin`) и admin CRUD (backend
  ...
  ручной приемкой на момент commit `7761901`, см. "Historical production
...
- HSTS включён (Stage 4) на HTTPS-ответах основного сайта
  ...
  сертификата Let's Encrypt настроено и проверено на текущем production
  VPS (`certbot.timer` + webroot + deploy hook, см. "Статус проекта" →
  "Текущая production-приемка" выше) — сам репозиторий эту автоматизацию
  по-прежнему не устанавливает, это операционная настройка хоста.
  Watchtower удален из инфраструктуры (Stage 3, см. "Почему так") —
  обновления образов только явные, ручные.
...
претендует на полную production-readiness (нет CD/автоматического деплоя —
обновления production по-прежнему выполняются оператором вручную по
процедуре из [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md); автопродление TLS
настроено вручную на хосте, а не автоматизацией этого репозитория, см.
"Статус проекта").
...
тесты"/"CI"). Полная ручная функциональная E2E-приемка (CRUD, заявки,
behavior metrics, аналитика и т.д.) пройдена по состоянию на commit
`7761901` на прежнем VPS (см. "Статус проекта" выше и "Ручная end-to-end
приемка"). Отдельно, 2026-09-12, на текущем VPS пройдена
production-деплой/smoke-приемка (lifecycle/readiness, auth/login/me, SPA,
периметр, TLS/автопродление, Registry push/pull, статус контейнеров) на
коммите `9b06ad6` (см. "Статус проекта" → "Текущая production-приемка") —
без повторения полного функционального E2E-сценария.
...
3. ~~Автоматизировать продление сертификата Let's Encrypt~~ — сделано на
   текущем production VPS (`certbot.timer`, webroot-аутентификатор,
   deploy hook — см. "Статус проекта" → "Текущая production-приемка" и
   [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)) в рамках миграции 2026-09-12;
   настроено вручную на хосте, не автоматизацией этого репозитория.
