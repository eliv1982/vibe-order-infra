...
стадии зафиксированы техническим baseline `39d355c` (`fix: finalize
repository review readiness`, коммит поверх `7cbe511`) — портфолио-
...
### Последняя VPS-приемка (commit `7761901`)
...
### Текущее репозиторное состояние (технический baseline `39d355c`)
...
- **HSTS** включен (Stage 4) в текущей конфигурации Nginx — см. "Security
  ...
  приемке `7761901` (см. выше) HSTS ещё не был включен.
- Backend + frontend покрыты тестами локально: backend suite на реальной
  ...
  `7761901`, а также не обязательно совпадают с тем, что дал бы прогон
  непосредственно на родительском коммите `7cbe511`.
...
- **Production delivery**: ни один production release, включая самый
  ...
  релизу, зафиксированному как commit `7761901` (последняя задокументированная
